...
export type GlobalSettings = {
  ...
  /**
   * 'number': The measurement is slowed-down by this factor. For scaling factors between zero and one, the simulation is accelerated accordingly. For example, if you enter the value 0.1 the measurement is accelerated by a factor of 10.
   * 'as-fast-as-possible': Run the simulation as fast as possible. The simulation speed is not constant and depends on the performance of the computer and the load that is provoked by simulation.
   */
...
export type Full6 = {
   ...
   * 'manual':Explicit definition of the key slot properties.
   */
  mode: 'automatic' | 'off' | 'manual' | string;
  /**
   * The frame in this slot can be send as start-up or only as sync frame.
   */
  usage?: ('startup' | 'sync') | string;
  /**
   * Explicit slot number of the static segment that will be used as the key slot.
   */
  slot?: number | string;
  /**
   * Explicit channel mask used for the key slot.
   */
  mask?: ('A' | 'B' | 'AB') | string;
  /**
   * If deactivated the key slot can only act as a following start-up or integrating sync frame.
   */
