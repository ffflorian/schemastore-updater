...
export type CANOrCANFDNetwork =
  | {
      ...
      /**
       * Index of the application channel. Value between 1 and 255. Value "0" is not allowed.
       */
      'application-channel'?: number | string;
      /**
       * Activates the generation of an acknowledgment when transmitting a CAN message via Vector network interfaces.
       * This allows the simulation of a network without connecting other active CAN control units.
       * In this case, the CAN channel used must still be terminated by an electric load resistor.
       */
      'tx-self-ack'?: boolean | string;
    }
  | {
      /**
       * A unique name that can be used to reference this CAN/CAN FD network in the configuration file or in programming APIs such as CAPL.
...
export type LoggingBlock =
  | {
      /**
       * Name of a .blf, .mf4, .pcap or .pcapng frame logging file, supporting the field codes: {ComputerName}, {LocalTime}, {MeasurementIndex}, {MeasurementStart}, {IncSize|001|100MB}, {IncTime|001|01h00m}, {IncTrigger|001}, {TriggerCondition} and {IncTrigger|001}_{TriggerCondition}.
       * .mf4, .pcap and .pcapng are not supported for target architecture Linux64.
       */
      'file-name': (
        | {
            [k: string]: unknown | undefined;
...
export interface DoIPSettings {
   ...
   */
  'routing-activation-type'?: string | number;
  /**
   * Configures the tester to expect an ECU simulation to send diagnostic acknowledgments.
   */
  'expect-diag-ack-messages'?: string | boolean;
  addressing?: DoIPAddressing;
  reconnect?: DoIPReconnect;
}
/**
...
export interface DoIPAddressing {
  ...
  /**
   * Second logical address of the ECU for physical diagnostic requests used within the response messages addressed to the tester.
   */
  'logical-ecu-address-phys-2'?: string | number;
  /**
...
/**
 * Configuration how the tester communication channel reacts when the ECU closes the TCP connection.
 */
export interface DoIPReconnect {
  /**
   * Delay (in ms) until the channel tries to open a new TCP connection to the vehicle. If set to 0, the channel will not try to reestablish a connection.
   */
  'delay-ms'?: string | number;
  /**
   * Interval (in ms) after which the channel will close the connection just initiated, if the vehicle did not confirm the TCP connection.
   */
  'interval-ms'?: string | number;
  /**
   * Maximum number of reconnect attempts. If the value is 0, only one attempt is performed.
   */
  'max-attempts'?: string | number;
}
/**
...
export interface MainDevice {
  ...
  'xve-file-path': (
    ...
    string;
  /**
   * Absolute or relative path to an EtherCAT variable mapping (.map) file. Relative path specifications are resolved relative to the configuration file.
   */
  'map-file-path'?: (
    | {
        [k: string]: unknown | undefined;
...
export interface EthernetNetwork {
  ...
  mapping?: ('internal-simulator' | 'external-sil-kit' | 'external-hardware') | string;
  /**
   * Index of the application channel. Value between 1 and 32. Value "0" is not allowed.
   */
