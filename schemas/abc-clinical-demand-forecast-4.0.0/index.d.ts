 ...
 * @minItems 1
 */
export type DispensationIntervals = [number, ...number[]];
/**
...
/**
 * Default months from site activation until first enrollment (First Patient In lead time) for child regions. Null means inherit from parent. Defaults to 0 if no ancestor sets a value.
 */
...
export interface ABCClinicalDemandForecastJSONSchema {
  /**
   * Link to https://json.schemastore.org/abc-clinical-demand-forecast-4.0.0.json
   */
  $schema?: 'https://json.schemastore.org/abc-clinical-demand-forecast-4.0.0.json';
  planNotes: PlanNotes;
  analytics: {
    items: AnalyticsItem[];
    layouts: {
