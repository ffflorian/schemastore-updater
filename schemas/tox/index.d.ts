...
/**
 * substitute the value of another configuration key
 */
...
/**
 * substitute the positional arguments passed to tox
 */
...
/**
 * substitute matches of a filesystem glob pattern
 */
...
/**
 * conditional substitution based on env vars, factors, or env_name
 */
export interface ReplaceIf {
  replace: 'if';
  condition: string;
  then: unknown;
  else?: unknown;
...
/**
 * range factor group: expands to prefix+N for N in [start, stop]
 */
export interface FactorRangeDict {
  prefix: string;
  start?: number;
  stop?: number;
  default?: string;
