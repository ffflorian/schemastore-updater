...
/**
 * An array of paths to JavaScript files, relative to manifest.json, that will be injected into the matching context.
 */
export type MailScriptScripts = string[];

/**
 * The manifest.json file is a JSON-formatted file that specifies basic metadata about a Thunderbird MailExtension such as its name and version, and can also specify aspects of the extension's functionality (such as compose scripts, message display scripts, background scripts, content scripts and cloud file providers).
 *
 * It is allowed to contain "//"-style comments.
 *
 * Supported manifest keys: https://developer.thunderbird.net/add-ons/mailextensions/supported-manifest-keys
 * Official manifest docs: https://developer.thunderbird.net/add-ons/mailextensions/manifest
 */
export interface ThunderbirdMailExtensionManifest {
  /**
   ...
   */
  version_name?: string;
  compose_action?: MailAction;
  message_display_action?: MailAction;
  cloud_file?: CloudFileProvider;
  oauth_provider?: OAuthProvider;
  compose_scripts?: MailScripts[];
  message_display_scripts?: MailScripts[];
...
/**
 * This key defines the appearance and behavior of an action button. Thunderbird provides two mail-specific variants: `compose_action` and `message_display_action`.
 */
export interface MailAction {
  /**
   * Enable browser styles. See the <a href='https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/user_interface/Browser_styles'>MDN documentation on browser styles</a> for more information.
   */
  browser_style?: boolean;
  /**
   * Defines the location the action button will appear in. The default location is `maintoolbar`.
   */
  default_area?: 'maintoolbar' | 'formattoolbar' | 'header' | 'context' | 'messageList';
  default_icon?: Icon;
  /**
   * The label of the button. This label is used as the accessible name of the button (for screen readers) and is displayed as tooltip.
   */
  default_label?: string;
  /**
   * The path to the HTML file that defines the popup. The file is opened when the user clicks the button. The path is relative to the manifest.json file.
   */
  default_popup?: string;
  /**
   * The title of the button, shown as tooltip when the user hovers over the action button.
   */
  default_title?: string;
  /**
   * This property lets you specify alternative icons to use, depending on whether Thunderbird detects that the theme uses dark mode.
   */
  theme_icons?: {
    /**
     * A URL pointing to an icon.
     */
    light: string;
    /**
     * A URL pointing to an icon.
...
/**
 * Defines a cloud file provider for uploading large attachments to a remote server. When a user clicks on the cloud file provider in the cloud file picker, the upload process is started.
 *
 * https://developer.thunderbird.net/add-ons/mailextensions/manifest/cloudfile
 */
export interface CloudFileProvider {
  /**
   * Enable browser styles in the `management_url` page. See the <a href='https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/user_interface/Browser_styles'>MDN documentation on browser styles</a> for more information.
   */
  browser_style?: boolean;
  /**
   * NO LONGER USED. This property is no longer used. The only supported data format for the `data` argument in the `onFileUpload` event of the `cloudFile` API is <a href='https://developer.mozilla.org/en-US/docs/Web/API/File'>File</a>.
   */
  data_format?: string;
  /**
  ...
  name?: string;
  /**
   * This property is no longer used. Previously, it transported the URL for creating a new account of this cloud file provider.
   */
  new_account_url?: string;
  /**
   * If a previously uploaded cloud file attachment is reused at a later time in a different message, Thunderbird may use the already known `url` and `templateInfo` values without triggering the registered `onFileUpload` listener again. Setting this option to `false` will always trigger the registered listener.
   */
  reuse_uploads?: boolean;
  /**
 ...
 *
 * https://developer.thunderbird.net/add-ons/mailextensions/manifest/oauth_provider
 */
export interface OAuthProvider {
  ...
  /**
   * Hostnames of the web pages, which are allowed to open a browser to begin the OAuth flow.
   */
  hostnames?: string[];
  /**
  ...
  issuer?: string;
  /**
   * Identifier of the OAuth2 issuer.
   */
  issuerIdentifier?: string;
  /**
   * Space-delimited string that specifies whether the authorization server prompts the user for re-authentication and consent.
   */
  prompt?: 'none' | 'login' | 'consent' | 'select_account';
  /**
   * Whether the user is prompted for consent.
   */
  promptConsent?: boolean;
  /**
   * OAuth redirect endpoint address (usually the same as the authorizationEndpoint).
   */
  redirectionEndpoint?: string;
  /**
   * OAuth scopes. The extension can provide a set of scopes to the OAuth authorization flow.
   */
  scopes?: string[];
  /**
  ...
  /**
   * If set to true, the OAuth authorization flow runs in the default browser instead of an embedded browser.
   */
  useExternalBrowser?: boolean;
  /**
   * If set to true, the OAuth authorization flow uses a PKCE challenge.
   */
  usePKCE?: boolean;
  [k: string]: unknown | undefined;
}
/**
 * Loads a script into one of the mailbox contexts (compose scripts run in the compose window, message display scripts run in the message display window).
 */
export interface MailScripts {
  js?: MailScriptScripts;
  css?: MailScriptScripts;
  /**
   * Determines when the scripts are loaded. The default value is `document_end`.
   */
