...
/**
 * Extends the standard tsconfig schema with the tsbuild property.
 */
export type TsbuildTsconfigSchemaExtension = JSONSchemaForAJavaScriptProjectUsingTypeScriptTooling & {
  tsbuild?: Options;
...
export interface Options {
  ...
  /**
   * Target platform. Auto-detected from tsconfig lib (DOM = browser, no DOM = node). Override if needed.
   */
  platform?: 'node' | 'browser' | 'neutral';
  /**
   * Whether to bundle source files together. When false, each file is output separately.
   */
  bundle?: boolean;
  /**
  ...
  clean?: boolean;
  /**
   * Default behavior for node_modules dependencies. 'bundle' includes them in output, 'external' keeps them as imports. When omitted, tsbuild auto-selects based on platform and noExternal.
   */
  packages?: 'bundle' | 'external';
  /**
   * Specific dependencies to externalize (don't bundle). In tsconfig.json this must be strings (RegExp patterns are only supported via the programmatic API).
   */
  external?: string[];
  /**
   * Specific dependencies to bundle (overrides packages setting). In tsconfig.json this must be strings (RegExp patterns are only supported via the programmatic API).
   */
  noExternal?: string[];
  dts?: {
    /**
     * Names of the project's entryPoints to be used to generate the DTS files
     */
    entryPoints?: string[];
    /**
     * Bundle external types from node_modules into declaration files. When false, external imports remain as import statements. Defaults to false for 'node' platform, true for 'browser' and 'neutral' platforms.
     */
    resolve?: boolean;
  };
  /**
   * Environment variables to define (accessible via import.meta.env)
   */
  env?: {
    [k: string]: string | undefined;
  };
  ...
  minify?: boolean;
  /**
   * Generate source maps. Options: true/false, 'inline', 'external', or 'both'. Overrides tsconfig.json compilerOptions.sourceMap.
   */
  ...
  splitting?: boolean;
  /**
   * Insert content at the beginning of generated files (e.g., shebang for CLI tools)
   */
  banner?: {
    /**
     * Banner text for JavaScript files
     */
    js?: string;
    /**
     * Banner text for CSS files
     */
    css?: string;
  };
  /**
   * Insert content at the end of generated files (e.g., copyright notices)
   */
  footer?: {
    /**
     * Footer text for JavaScript files
     */
    js?: string;
    /**
     * Footer text for CSS files
     */
    css?: string;
  };
  plugins?: (
