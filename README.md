...
`npm stage publish` cannot create a package for the first time - it fails for any schema that has never been published before, regardless of OIDC or token auth, since npm requires a package to already exist before it can be staged or trusted. To bootstrap a brand-new schema, trigger the publish workflow manually with the `bootstrap_packages` input set to its schema ID(s) (space or comma separated). That runs a plain `npm publish` for each one with no credentials configured, so npm prints a browser login URL into the job log - open it on any device (a phone works fine) and approve with 2FA to let the publish continue. Once a schema exists on npm this way, configure npm Trusted Publishing for it and this pipeline takes over automatically for future versions.
