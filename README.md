...
`npm stage publish` cannot create a package for the first time - it fails for any schema that has never been published before, regardless of OIDC or token auth, since npm requires a package to already exist before it can be staged or trusted. To bootstrap brand-new schemas, trigger the publish workflow manually with the `bootstrap_new_packages` input set to true. That runs `yarn bootstrap:new-packages`, which finds every not-yet-published schema, checks the public registry to tell which ones have genuinely never been published before, and runs a plain `npm publish` for just those - no schema IDs to type in. With no credentials configured, npm prints a browser login URL into the job log - open it on any device (a phone works fine) and approve with 2FA to let the publish(es) continue; one approval covers the whole run. Once a schema exists on npm this way, configure npm Trusted Publishing for it and this pipeline takes over automatically for future versions.
