...
- An `NPM_TOKEN` secret exists as a fallback for packages that already exist on npm but have no Trusted Publisher configured yet. It is only used when the OIDC-only attempt fails, and it can never bypass 2FA since `npm stage publish` never requires 2FA regardless of auth method.
- `npm stage publish` cannot create a package for the first time - it 404s for any schema that has never been published before, regardless of OIDC or token auth. A schema's very first version must still be published directly (`npm publish`, not staged) before it exists on npm; `yarn bootstrap:new-packages` then configures the Trusted Publisher automatically (see below) so this pipeline can pick up future versions. CI failures for brand-new schemas are expected until the first publish happens, not an infra fix.
- Brand-new schemas are bootstrapped with `yarn bootstrap:new-packages` (`bootstrapNewPackages()` in `src/publisher.ts`), but **only from a maintainer's own terminal, never from CI**. It scans every non-blocklisted, not-yet-`published` schema package, checks the public registry directly (no auth needed) to tell brand-new packages apart from existing ones with just a pending new version, and for each true first-ever-publish candidate runs `npm login --auth-type=web` once (printing a browser login URL - open it on any device, a phone works fine, and approve with 2FA/passkey), then a plain `npm publish` per candidate with stdio inherited from the calling terminal.
  - This cannot be automated in CI: an account with 2FA required for publish (which is required going forward - see below) challenges the publish action itself with a one-time password (`EOTP`), and `npm publish` only resolves that challenge interactively (a real TTY). In a non-TTY environment it just prints the challenge and exits.
  - We tried reimplementing npm's own `authUrl`/`doneUrl` web-OTP polling protocol from a wrapping script (multiple attempts, including authenticating the poll as the logged-in session) and it never got past a bare `404` on `doneUrl`. This is not unique to this repo: `changesets/changesets#1773` is an open, unimplemented feature request for exactly this (non-interactive web-OTP for WebAuthn/passkey accounts), and npm's own CLI (`lib/utils/auth.js`) explicitly skips attempting this poll at all when stdin/stdout aren't a TTY. Do not re-attempt CI-side OTP polling without new evidence it's actually solvable.
  - npm has no PyPI-style "pending trusted publisher" (registering OIDC trust for a package that doesn't exist yet) - namespace-wide trusted publishing to fix this is on npm's own roadmap but not shipped. Until it ships, a brand-new package's first version always needs one interactive, human-approved `npm publish`, run locally.
  - Each successful bootstrap is marked `published: true` immediately so the normal staged-publish step doesn't re-attempt the same version. Existing packages with just a pending new version are left untouched for that step to stage as usual.
  - Immediately after each successful `npm publish`, `bootstrapNewPackages()` calls `configureNpmTrustedPublisher()`, which runs `npm trust github <package> --allow-stage-publish --repo ffflorian/schemastore-updater --file publish_generated_packages.yml --yes` (granting CI's OIDC identity `npm stage publish` only, never a direct `npm publish`) and then `npm access set mfa=publish <package>` (disallowing token-based publish entirely, so only the trusted workflow or an interactive, 2FA-authenticated `npm publish` can publish this package from here on).
  - Both of those npm commands may themselves challenge for an OTP. By default this is left interactive (same `spawnNpmWithInheritedStdio` pattern as login/publish, so the prompt appears in the calling terminal). Setting the `NPM_OTP_COMMAND` environment variable to a shell command that prints a fresh OTP to stdout (e.g. wrapping a local TOTP CLI) supplies `--otp=<code>` non-interactively instead; it is invoked separately before each of the two commands since each code is single-use.
  - If `configureNpmTrustedPublisher()` fails, the package is still recorded as bootstrapped/published (that part already happened and can't be undone) - a warning is printed telling the maintainer to configure the Trusted Publisher and "Require two-factor authentication and disallow tokens" manually on npmjs.com instead. This must never roll back `published: true` or otherwise be treated as a publish failure.
...
- Bootstrap brand-new schema packages (run locally only, never in CI): `yarn bootstrap:new-packages`
