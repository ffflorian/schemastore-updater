...
- `npm stage publish` cannot create a package for the first time - it 404s for any schema that has never been published before, regardless of OIDC or token auth. A schema's very first version must still be published directly (`npm publish`, not staged) before it exists on npm; only then can a Trusted Publisher be configured for it and this pipeline pick up future versions automatically. CI failures for brand-new schemas are expected until this happens, not an infra fix.
- The same workflow has a manual `bootstrap_new_packages` `workflow_dispatch` boolean input for exactly this first-ever-publish case. When true, a step runs `yarn bootstrap:new-packages` (`bootstrapNewPackages()` in `src/publisher.ts`) before the normal publish step. It scans every non-blocklisted, not-yet-`published` schema package, checks the public registry directly (no auth needed) to tell brand-new packages apart from existing ones with just a pending new version, and only if it finds at least one true first-ever-publish candidate, runs `npm login --auth-type=web` once. `npm publish` does **not** fall back to a browser login from a fully unauthenticated state on its own (confirmed: it fails immediately with `ENEEDAUTH`) - only `npm login` does, so login has to be its own explicit step, done once, before the plain `npm publish` calls that follow for each candidate. `npm login --auth-type=web` (the npm >=9 default auth type for the public registry) prints a login URL into the job log, which a maintainer opens and approves (2FA included) from any device, e.g. a phone, with no local npm CLI or console needed. The resulting session is cached for the rest of that step, so bootstrapping several packages in one dispatch only needs one _login_ approval. Accounts that require 2FA to publish additionally challenge the publish action itself with a one-time password (`EOTP`), separate from being logged in - `npm publish` does not poll for that approval on its own in a non-TTY environment (confirmed: it just prints an `authUrl`/`doneUrl` pair and exits), so `publishNewPackageDirectory` parses those URLs out of npm's stderr, prints the `authUrl` for a maintainer to open and approve, polls `doneUrl` itself (`GET`, `202` = pending with a `retry-after` header, `200` = done with a `token` field - the same protocol `npm login --auth-type=web` uses internally), and retries the publish once with that token via a temporary `--userconfig`. This OTP approval is a separate URL per package (unlike the login step, it is not known to be cached across packages in the same run). Each successful bootstrap is marked `published: true` immediately so the normal staged-publish step right after doesn't re-attempt the same version. Existing packages with just a pending new version are left untouched for that normal step to stage as usual.
