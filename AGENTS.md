...
- `npm stage publish` cannot create a package for the first time - it 404s for any schema that has never been published before, regardless of OIDC or token auth. A schema's very first version must be published manually by a maintainer (outside this pipeline, with their own npm login) before it exists on npm; only then can a Trusted Publisher be configured for it and this pipeline pick up future versions automatically. CI failures for brand-new schemas are expected and require this manual step, not an infra fix.
