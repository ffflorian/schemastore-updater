...
- The same workflow has a manual `bootstrap_packages` `workflow_dispatch` input (space/comma-separated schema IDs) for exactly this first-ever-publish case. When set, a step runs a plain `npm publish` for each listed schema before the normal publish step, with no credentials configured in that job - npm's default browser-based login (`--auth-type=web`, the npm >=9 default for the public registry) prints a login URL into the job log, which a maintainer opens and approves (2FA included) from any device, e.g. a phone, with no local npm CLI or console needed. Login is cached for the rest of that step, so multiple packages in one dispatch only need one approval. Each successful bootstrap is marked `published: true` via `yarn mark-published <schema>` so the normal staged-publish step doesn't immediately re-attempt the same version.
